...
def log(event_number: int = 1, message: str = "Log вызван без аргументов.", console=False, time=False):
    """
    Запись события в log-файл (вывод на консоль,
    при вклчюенном console = True).
    """
    ...
    if event_number not in events:
        events[event_number] = f"Отсутствует событие с ключом: {event_number}"
    # message с неверным типом данных.
    if not type(message) is str:
        message = "При вызове в ф-цию Log отправлен не верный тип. Нужен тип: str"
